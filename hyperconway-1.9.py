...
def detect_period():
    ...
    for generation in range(256):
        ...
        if len(current_generation) >= pop_threshold:
            ...
            minpop = min(pops[period_universe.index(set(current_generation)):])
            ...
            pops.append(len(current_generation))
    P = Toplevel(C)
    P.geometry(window_size)
    P.title("Detect period")
    Label(P, text= "No periodicity detected within " + str(gen_threshold) + " generations", font=("Arial", int(scale/50), "bold")).place(x=0.01*scale, y=0.01*scale)
    Label(P, text= "Lowest population reached: " + str(min(pops)) + ", highest population reached: " + str(max(pops)), font=("Arial", int(scale/70), "bold")).place(x=0.01*scale, y=0.045*scale)
    return
...
def buttons():
    ...
    periodbutton = Button(C, font=("Arial", int(scale/70), "bold"), text="Detect period", command = lambda: [detect_period()])
    ...
    historybutton.place(x=0.03*scale, y=0.91*scale)
    periodbutton.place(x=0.03*scale, y=0.95*scale)
